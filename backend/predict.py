...
def predict(
    ...
    config["n_subjects"] = n_subjects


    subject_files = glob.glob(os.path.join("./uploads/", dataset, "*.npz"))
    print("SSSSSSSSSSSSSSSs",subject_files)
    
    # Load subject IDs
    fname = "{}.txt".format(config["dataset"])
    seq_sids = np.asarray(range(n_subjects))
    print("fname",fname)
    print("seq_sids",seq_sids)
    logger.info("Load generated SIDs from {}".format(fname))
    outp += "Load generated SIDs from {}\n".format(fname)
    logger.info("SIDs ({}): {}".format(len(seq_sids), seq_sids))
    outp += "SIDs ({}): {}\n".format(len(seq_sids), seq_sids)

    # Split training and test sets
    fold_pids = np.array_split(seq_sids, config["n_folds"])
    print("fold_pids",fold_pids)
        ...
        for sid in test_sids:
            ...
            if config["model"] == "model-origin":
                ...
                for night_idx, night_data in enumerate(zip(test_x, test_y)):
                    ...
                    s_trues.extend(test_outs["test/trues"])
                    s_preds.extend(test_outs["test/preds"])
                    preds_cur = test_outs["test/preds"]
                    trues.extend(test_outs["test/trues"])
                    preds.extend(test_outs["test/preds"])
    ...
    outp += "Total: {}\n".format(metrics[0])
    outp += "Number of samples from each class: {}\n".format(metrics[1])
    outp += "Accuracy: {:.1f}\n".format(metrics[2]*100.0)
    outp += "Macro F1-Score: {:.1f}\n".format(metrics[3]*100.0)
    outp += "Per-class Precision: " + " ".join(["{:.1f}\n".format(m*100.0) for m in metrics[4]])
    outp += "Per-class Recall: " + " ".join(["{:.1f}\n".format(m*100.0) for m in metrics[5]])
    outp += "Per-class F1-Score: " + " ".join(["{:.1f}\n".format(m*100.0) for m in metrics[6]])
